...
#import "TDWXPaySignAdaptor.h"
#import <CommonCrypto/CommonDigest.h>

@interface TDWXPaySignAdaptor()
...
@implementation TDWXPaySignAdaptor

#pragma mark - Life Cycle

#pragma mark --下订单时候首次签名----
                          ...
                          tradeType:(NSString *)tradeType
{
    if (self = [super init]) {
        ...
        [self.dic setValue:[self createMd5Sign:self.dic] forKey:WXSIGN];
    }
    return self;
}
                       ...
                       timestamp:(UInt32)timestamp_key
{
    ...
    //添加商户密钥key字段
    [contentString appendFormat:@"key=%@", TDWechatPartnerKey];
                              ...
                              orderNo :(NSString *)orderNo
{
    ...
    [paramsDic setValue:wechatAppId forKey:WXAPPID];
    [paramsDic setValue:wechatMCHId forKey:WXMCHID];
    [paramsDic setValue:wechatPartnerKey     forKey:WXNONCESTR];
    [paramsDic setValue:tradeNo     forKey:WXNONCESTR];
    [paramsDic setValue:orderNo     forKey:WXOUTTRADENO];
    [paramsDic setValue:[self createMd5Sign:paramsDic]     forKey:WXSIGN];
