...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        if (indexPath.row == 0) {
            ...
            cell.titleLabel.text = NSLocalizedString(@"添加好友", @"添加好友");
        }
        if (indexPath.row == 1) {
            cell.avatarView.image = [UIImage imageNamed:@"EaseUIResource.bundle/group"];
            cell.titleLabel.text = NSLocalizedString(@"好友申请", @"好友申请");
        }
        if (indexPath.row == 2) {
            cell.avatarView.image = [UIImage imageNamed:@"EaseUIResource.bundle/group"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (section == 0) {
        if (row == 0)
        {
            TDAddFriendViewController *groupController = [[TDAddFriendViewController alloc] init];
            [self.navigationController pushViewController:groupController animated:YES];
        }
       else if (row == 1)
        {
            TDApplyTableViewController *groupController = [[TDApplyTableViewController alloc] init];
            [self.navigationController pushViewController:groupController animated:YES];
        }
      else if (row == 2)
        {
