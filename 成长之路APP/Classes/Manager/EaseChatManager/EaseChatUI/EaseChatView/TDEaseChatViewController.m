...
@interface TDEaseChatViewController ()<UIAlertViewDelegate,EMClientDelegate /*,EMChooseViewDelegate*/>
{
...
@implementation TDEaseChatViewController
...
{
   self.navigationItem.leftBarButtonItem = [UIBarButtonItem leftBarButtonItemWithImage:[UIImage imageNamed:@"返回"] highlighted:[UIImage imageNamed:@"返回"] target:self selector:@selector(backAction)];
               ...
               selectAtTarget:(EaseSelectAtTargetCallback)selectedCallback
{
    ...
    if (chatGroup) {
        if (!chatGroup.occupants) {
            __weak TDEaseChatViewController* weakSelf = self;
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    __strong TDEaseChatViewController *strongSelf = weakSelf;
                    if (strongSelf) {
                        [strongSelf hideHud];
                        if (error) {
